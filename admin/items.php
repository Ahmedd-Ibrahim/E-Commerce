...
if (isset($_SESSION['username'])) {
            ...
            foreach ($fetchitems as $item) {
              ...
              <tbody>
                <tr>
                  ...
                  <td>
                    <a href="?do=delete&id=<?php echo $item['item_id']; ?>" class="btn btn-danger edit"><i class="fas fa-trash-alt"></i> Delete</a>
                    <a href="?do=edit&id=<?php echo $item['item_id']; ?>" class="btn btn-success edit"><i class="fas fa-edit"></i> Edit </a>
                    <?php if ($item['status'] == 0) {  ?>
                      <a href="items.php?do=active&id=<?php echo $item['item_id']; ?>" class="btn btn-info "><i class="fas fa-check-circle"></i> active</a>
                    <?php
                    } else {
                      ?>
                      <a href="items.php?do=deactive&id=<?php echo $item['item_id']; ?>" class="btn btn-info "><i class="fas fa-times-circle"></i> deactive</a>
                    <?php
                    }
                    ?>
                  </td>
  ...
  <?php
    ...
    if ($rowCount !== 0 && isset($_SERVER['HTTP_REFERER'])) {  ?>
      <!-- Form Edit item -->
      <form class="form-group edit-form" action="?do=update&id=<?php echo $_GET['id'] ?>" method="POST"  enctype="multipart/form-data">
        <form class="form-group add-form" action="?do=insert" method="POST">
          <div class="container">
            ...
            <div class="form-group row">
              <div class="col-sm-5">
                <button type="submit" name='update' class="btn btn-primary btn-block btn-lg"><i class="fas fa-edit"></i> Edit item</button>
              </div>
            </div>
          </div>
        </form>
    ...
    if (isset($_POST['update']) && $do == 'update') { // check if set update
      ...
      if (empty($oneError) && $_SERVER['REQUEST_METHOD'] == 'POST') { // if no error and method POST insert this user
        ...
        if ($catExist == true) {  // if categories exists display an error message and stop script
          ...
          $q = "UPDATE `items` SET name=:name, description=:description, 
          price=:price, status=:status, country_made=:country, user_id=:user_id, 
          cat_id=:cat_id, add_date=now(), avater_item=:avater_item WHERE item_id=:item_id";
